...
func buildThinkerPrompt(task string) string {
	...
	return strings.Join([]string{
		...
		"tek maddede topla — uygulayan onu açar.",
		"",
		"=== GÖREV ===",
		task,
	}, "\n")
...
func buildWriterPrompt(originalTask, thinkerOutput string) string {
	if Lang() == LangEN {
		return strings.Join([]string{
			...
			"for approval to run them — the user verifies. Finish with one sentence saying",
	...
	return strings.Join([]string{
		...
		"için onay isteme — doğrulamayı kullanıcı yapar. Bitirince ne oluşturduğunu",
		"tek cümleyle yaz.",
		"",
		"=== ORİJİNAL GÖREV ===",
		originalTask,

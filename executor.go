...
func buildThinkerPrompt(task string) string {
	if Lang() == LangEN {
		return strings.Join([]string{
			...
			"",
			"Keep the whole plan under 15 lines. Group the test cases into a single",
			"bullet instead of listing them one by one — the implementer expands it.",
			"Only plan tests if the task actually asked for them: a test file is",
			"usually larger than the code itself, and the implementer writes every",
			"line of it.",
			"",
			"=== TASK ===",
			task,
	...
	return strings.Join([]string{
		...
		"",
		"Planın tamamı 15 satırı geçmesin. Test senaryolarını tek tek saymak yerine",
		"tek maddede topla — uygulayan onu açar. Görev açıkça test istemediyse",
		"plana test maddesi KOYMA: test dosyası çoğu zaman koddan büyük oluyor ve",
		"her satırını uygulayan yazıyor.",
		"",
		"=== GÖREV ===",
		task,
	}, "\n")
...
func buildWriterPrompt(originalTask, thinkerOutput string) string {
	if Lang() == LangEN {
		return strings.Join([]string{
			...
			"no exploration the plan did not ask for.",
			"",
			"Write files only. Do NOT run commands (tests, build, go run) and do not ask",
			"for approval to run them — the user verifies.",
			"",
			"Write the leanest code that works: no decorative comments, no docstrings",
			"repeating the signature, no file the plan did not ask for. Finish with one",
			"sentence saying what you created or changed.",
			"",
			"=== ORIGINAL TASK ===",
			originalTask,
			"",
	...
	return strings.Join([]string{
		...
		"istemediği keşif yok.",
		"",
		"Sadece dosyaları yaz. Komut ÇALIŞTIRMA (test, build, go run) ve çalıştırmak",
		"için onay isteme — doğrulamayı kullanıcı yapar.",
		"",
		"En yalın çalışan kodu yaz: süs yorumu yok, imzayı tekrarlayan docstring yok,",
		"planın istemediği dosya yok. Bitirince ne oluşturduğunu tek cümleyle yaz.",
		"",
		"=== ORİJİNAL GÖREV ===",
		originalTask,
		"",
